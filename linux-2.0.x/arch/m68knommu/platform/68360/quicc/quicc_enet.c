/*
 * I have not tested or debugged this code. All I know is it compiles into the
...
static char rx_buf_pool[NUMBER_ENET_RX_BUFFERS * CPM_ENET_RX_FRSIZE];
...
struct scc_enet_private {
	...
	struct	sk_buff* tx_skbuff[NUMBER_ENET_TX_BUFFERS];
	ushort	skb_cur;
	ushort	skb_dirty;
...
scc_enet_timeout(struct net_device *dev)
{
...
#ifndef final_version
	{
		...
		printk(" Ring data dump: cur_tx %p%s cur_rx %p.\n",
		       ...
		       cep->cur_rx);
		bdp = cep->tx_bd_base;
		for (i = 0 ; i < NUMBER_ENET_TX_BUFFERS; i++, bdp++)
			printk("%04x %04x %08x\n",
			       bdp->status,
			       bdp->length,
			       (int)(bdp->buf));
		bdp = cep->rx_bd_base;
		for (i = 0 ; i < NUMBER_ENET_RX_BUFFERS; i++, bdp++)
			printk("%04x %04x %08x\n",
			       bdp->status,
			       bdp->length,
			       (int)(bdp->buf));
...
static void scc_enet_interrupt(int vec, void *dev_id, struct pt_regs *fp)
{
	...
	*/
	int_events = cep->sccp->scce;
	cep->sccp->scce = int_events;
	must_restart = 0;
...
int scc_enet_init(void)
{
	...
	*/
	sccp->gsmr_l &= ~(SCC_GSMRL_ENR | SCC_GSMRL_ENT);
	...
	*/
	 ...
	 */
	/*i = m360_cpm_dpalloc(sizeof(QUICC_BD) * NUMBER_ENET_RX_BUFFERS); */
        i = pquicc->ch_or_u.mem_config.enet_rx_bd;
	ep->rbase = i;
	cep->rx_bd_base = (QUICC_BD *)((uint)pquicc + i);

	/*i = m360_cpm_dpalloc(sizeof(QUICC_BD) * NUMBER_ENET_TX_BUFFERS); */
        i = pquicc->ch_or_u.mem_config.enet_tx_bd;
	ep->tbase = i;
	...
	*/
	 ...
	 */
#if defined (CONFIG_UCQUICC) || defined(CONFIG_SED_SIOS)
	{
...
/* #endif */
	...
	bdp = cep->tx_bd_base;
	for (j=0; j<(NUMBER_ENET_TX_BUFFERS-1); j++) {
		bdp->buf = 0;
		bdp->status = 0;
		bdp++;
	}
	...
	/* initialize rx buffer descriptors */
	bdp = cep->rx_bd_base;
	for (j=0; j<(NUMBER_ENET_RX_BUFFERS-1); j++) {
	...
	sccp->scce = 0xffff;	/* Clear any pending events */

	/* Enable interrupts for transmit error, complete frame
	 * received, and any transmit buffer we have also set the
	 * interrupt flag.
	 */
	sccp->sccm = (SCCE_ENET_TXE | SCCE_ENET_RXF | SCCE_ENET_TXB);
	 ...
	 */
	sccp->gsmr_h = 0;
	sccp->gsmr_l = (SCC_GSMRL_TCI | SCC_GSMRL_TPL_48 |
	 ...
	 */
	sccp->psmr = (SCC_PMSR_ENCRC       /* Ethernet CRC mode */
