 ...
 *               In this function this statement is commented out.
 *
 *		*(volatile unsigned char *)0xfffff207 = 0xc0;  idle the cpu 
 *              void show_regs(struct pt_regs * regs
 *                      This function is commented out. This is required
...
{
	/*
	 * Posh2 - This is required only for debugging purpose.
...
/*
 * "sh2_fork()".. By the time we get here, the
 * non-volatile registers have also been saved on the
 * stack. We do some ugly pointer stuff here.. (see
 * also copy_thread)
 */
asmlinkage int sh2_fork(struct pt_regs *regs)
{
	return do_fork(SIGCHLD|CLONE_WAIT, rdusp(), regs);
}

asmlinkage int sh2_clone(struct pt_regs *regs)
{
		 ...
		 struct task_struct * p, struct pt_regs * regs)
{
	...
	stack_offset = ((24*1024) - sizeof(struct pt_regs));
	...
	*childstack = *stack;
	/* 
	 * posh2 - Commenting out for the time being
	 */
	 childstack->retpc = (unsigned long) ret_from_exception;

	/*p->tss.usp = usp;*/ /*Posh2 dividing kernel stack for usp and ksp*/
	stack_offset =   20*1024;  //8192 >> 1;
	p->tss.usp =  (p->kernel_stack_page + stack_offset);
