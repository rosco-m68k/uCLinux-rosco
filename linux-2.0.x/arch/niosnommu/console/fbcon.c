/*
 * linux/arch/niosnommu/console/fbcon.c -- Low level frame buffer based console
 *                                    driver
 * Based on: 
 *
...
extern struct fb_info *e1355fb_init(void); // wentao
...
#define CONFIG_FBCON_8PACKED  // wentao
#define CONFIG_FBCON_16PACKED // wentao
#define __LITTLE_ENDIAN //wentao
/* Amiga support */
...
static u_long fbcon_startup(u_long kmem_start, char **display_desc)
{
   ...
   fb_info = e1355fb_init(); //(&kmem_start);
   disp = fb_info->disp;
...
/* wentao: from fb_memset255, set to FF for all bytes? */
static __inline__ void *mymemset(void *s, size_t count)
{
    unsigned long xs = (unsigned long) s;
...
static __inline__ void fast_memmove(char *d, char *s, size_t count)
{
    unsigned long dst, src;
...
extern u_short packed16_cmap[16];
