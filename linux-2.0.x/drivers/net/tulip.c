...
#endif /* CONFIG_COLDFIRE && CONFIG_PCI */
...
#ifdef __BIG_ENDIAN
#undef	le16_to_cpu
#define le16_to_cpu(w)          ((((w) & 0xff) << 8) | (((w) & 0xff00) >> 8))
#endif
...
struct tulip_private {
	...
	struct device *next_module;
        ...
        int rx_ring_size;
        int tx_ring_size;
        void *rx_comem[RX_RING_SIZE];
        void *tx_comem[TX_RING_SIZE];
        void *setup_framep;
#else
...
#if defined(CONFIG_COMEMPCI)
		if (tp->setup_framep == 0)
			tp->setup_framep = pci_bmalloc(sizeof(tp->setup_frame));
		pci_bmcpyto(tp->setup_framep, tp->setup_frame, sizeof(tp->setup_frame));
#endif

		/* Put the setup frame on the Tx list. */
		tp->tx_ring[0].length = cpu_to_le32(0x08000000 | 192);
#if defined(CONFIG_COMEMPCI)
		tp->tx_ring[0].buffer1 = virt_to_le32desc(tp->setup_framep);
#else
...
#if defined(CONFIG_COMEMPCI)
	/*
	 *      Allocate ring memory in PCI shared memory.
	 *      This simplifies the bus master aspect of it greatly...
	 */
	tp->rx_ring_size = sizeof(struct tulip_rx_desc) * RX_RING_SIZE;
	tp->rx_ring = (struct tulip_rx_desc *) pci_bmalloc(tp->rx_ring_size);
	tp->tx_ring_size = sizeof(struct tulip_tx_desc) * TX_RING_SIZE;
	tp->tx_ring = (struct tulip_tx_desc *) pci_bmalloc(tp->tx_ring_size);
#endif
...
#if defined(CONFIG_COMEMPCI)
		/* Keep track of PCI memory buffers */
		if (tp->rx_comem[i] == 0) 
			tp->rx_comem[i] = pci_bmalloc(PKT_BUF_SZ);
		tp->rx_ring[i].buffer1 = virt_to_le32desc(tp->rx_comem[i]);
#else
...
#if defined(CONFIG_COMEMPCI)
		if (tp->tx_comem[i] == 0)
			tp->tx_comem[i] = pci_bmalloc(PKT_BUF_SZ);
#endif
	}
...
#if defined(CONFIG_COMEMPCI)
    /* Copy packet data into PCI shared memory. */
    pci_bmcpyto(tp->tx_comem[entry], skb->data, skb->len);
    tp->tx_ring[entry].buffer1 = virt_to_le32desc(tp->tx_comem[entry]);
#else
...
#if defined(CONFIG_COMEMPCI)
            /* Keep track of PCI memory buffers */
            if (tp->rx_comem[entry] == 0)
                tp->rx_comem[entry] = pci_bmalloc(PKT_BUF_SZ);
            tp->rx_ring[entry].buffer1 = virt_to_le32desc(tp->rx_comem[entry]);
#else
...
#if ! defined(__alpha__)
#if defined(CONFIG_COMEMPCI)
                pci_bmcpyfrom(skb->tail, tp->rx_comem[entry], pkt_len);
				skb_put(skb, pkt_len);
#else
...
#if defined(CONFIG_COMEMPCI)
			tp->tx_ring[entry].buffer1 = virt_to_le32desc(tp->setup_framep);
#else
