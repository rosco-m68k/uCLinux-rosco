...
static int smc_ioctl(struct device *dev, struct ifreq *rq, int cmd);
...
static int smc_probe( unsigned int ioaddr ) 
{
	...
	if ( (bank & 0xFF00) != 0x3300 ) {
		PRINTK ((CARDNAME ": (1) Vendor ID doesn't match mfgr (%x).\n", bank));
		return -ENODEV;
	}
	...
	bank = inw( ioaddr + BANK_SELECT );
	if ( (bank & 0xFF00 ) != 0x3300 ) {
		PRINTK ((CARDNAME ": (2) Vendor ID doesn't match mfgr (%x).\n", bank));
		return -ENODEV;
	}
...
#elif defined(CONFIG_M68EZ328)
	irqval = request_irq(IRQ_MACHSPEC | dev->irq, &smc_interrupt,
			IRQ_FLG_STD, "SMSC_Lan91C96", NULL);
	if (irqval) panic("Unable to attach Lan91C96 intr\n");
#else
      	irqval = request_irq(dev->irq, &smc_interrupt, 0, CARDNAME, NULL);
#endif
      	if (irqval) {
 ...
 */
	...
	*/
	...
	ledman_cmd((inw(ioaddr + EPH_STATUS) & ES_LINK_OK) == ES_LINK_OK ?
			...
			((struct smc_local *) dev->priv)->interface_num ?
...
/* will Linux let this happen ??  If not, this costs some speed */
	...
	do { 	
		...
		if (status & IM_RCV_INT) {
			...
			ledman_cmd((inw(ioaddr + EPH_STATUS) & ES_LINK_OK) == ES_LINK_OK ?
					LEDMAN_CMD_ON : LEDMAN_CMD_OFF,
					lp->interface_num?LEDMAN_LAN2_LINK:LEDMAN_LAN1_LINK);
			SMC_SELECT_BANK( 1 );
			/* ACK it */
			outw(inw(ioaddr + CONTROL) & ~CTL_LE_ENABLE, ioaddr + CONTROL);	
			outw(inw(ioaddr + CONTROL) | CTL_LE_ENABLE, ioaddr + CONTROL);	
...
static int smc_ioctl(struct device *dev, struct ifreq *rq, int cmd)
{
  ...
  unsigned int ioaddr = dev->base_addr;
    ...
    switch (cmd) {
     case SMC_SIOCLINK:
       SMC_SELECT_BANK( 0 );
       linkok = (inw(ioaddr + EPH_STATUS) & ES_LINK_OK) == ES_LINK_OK;
