...
static volatile struct atmel_usart_regs *usarts[USART_CNT] = {
	(volatile struct atmel_usart_regs *) USART0_BASE,
	(volatile struct atmel_usart_regs *) USART1_BASE
};
static struct atmel_serial atmel_info[USART_CNT];
...
static unsigned char * rx_buf_table[USART_CNT];
...
{
	info->event |= 1 << event;
	queue_task_irq_off(&info->tqueue, &tq_atmel_serial);
	mark_bh(SERIAL_BH);
}

extern void breakpoint(void);	/* For the KGDB frame character */

static _INLINE_ void receive_chars(struct atmel_serial *info, u_32 status)
{
...
static _INLINE_ void status_handle(struct atmel_serial *info, u_32 status)
{
#if 0
	if (status & DCD) {
...
static void uart_init(struct atmel_serial *info)
{
	...
	/* Enable output instead of pio and clock the usart */
	APMC_BASE->APMC_PCER = (1 << US0_ID) ;
	PIOA_BASE->PIO_PDR = (1 << PIOTXD0) | (1 << PIORXD0);
#endif
...
{
	info->usart->tcr = 0;
	info->usart->tpr = (u_32) p;
	info->usart->tcr = (u_32) len;
	tx_start(info->usart, info->use_ints);
}
...
static void init_console(struct atmel_serial *info)
{
	...
	info->usart = (volatile struct atmel_usart_regs *) USART1_BASE;
	info->irqmask = AIC_URT1;
	info->irq = IRQ_USART1;
#else
	info->usart = (volatile struct atmel_usart_regs *) USART0_BASE;
	info->irqmask = AIC_URT0;
	info->irq = IRQ_USART0;
#endif
	info->tty = 0;
	info->port = 0;
					...
					unsigned int cmd, unsigned long arg)
{
	...
	case TIOCSERGSTRUCT:
		...
		if (error)
			return error;
		memcpy_tofs((struct atmel_serial *) arg, info,
			sizeof(struct atmel_serial));
		return 0;
...
static struct irqaction irq_usarta =
	{ rs_interrupta, 0, 0, "usarta", NULL, NULL };
...
{
	setup_arm_irq(IRQ_USART0, &irq_usarta);
	setup_arm_irq(IRQ_USART1, &irq_usartb);
}
...
int rs_atmel_init(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		info->usart = usarts[i];
		info->tty = 0;
		info->irqmask = (i) ? AIC_URT1 : AIC_URT0;
