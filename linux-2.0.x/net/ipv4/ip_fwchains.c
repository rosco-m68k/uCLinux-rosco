...
/* Pre 2.1 didn't have net_ratelimit(). */
#define net_ratelimit() 1

#define SLOT_NUMBER() (!intr_count)
#define NUM_SLOTS 2

/* 2.0.x: ``what's a spinlock?'' */
typedef struct { } spinlock_t;
#define read_lock(l) do { } while(0)
#define read_unlock(l) do { } while(0)
#define write_lock_irqsave(l,f) do { save_flags(f); cli(); } while(0)
#define write_unlock_irqrestore(l,f) do { restore_flags(f); } while(0)
...
#endif /*DEBUG_IP_FIREWALL_LOCKING*/
		     ...
		     const struct ip_fwkernel *rule)
{
	...
	tmp =  branchname(rule->branch,rule->simplebranch);
	len=sprintf(buffer,
		    "%9s "			/* Chain name */
...
outside:
	...
	duprintf("ip_chain_procinf: Length = %i (of %i).  Offset = %li.\n",
		 len, length, upto);
...
int ipfw_input_check(struct firewall_ops *this, int pf, struct device *dev, 
		     void *phdr, void *arg)
{
	return ip_fw_check(phdr, dev->name, arg, IP_FW_INPUT_CHAIN,
			   SLOT_NUMBER(), 0);
}
...
int ipfw_output_check(struct firewall_ops *this, int pf, struct device *dev, 
		      void *phdr, void *arg)
{
	return ip_fw_check(phdr, dev->name, arg, IP_FW_OUTPUT_CHAIN, 
			   SLOT_NUMBER(), 0);
}
...
int ipfw_forward_check(struct firewall_ops *this, int pf, struct device *dev, 
		       void *phdr, void *arg)
{
	return ip_fw_check(phdr, dev->name, arg, IP_FW_FORWARD_CHAIN, 
			   SLOT_NUMBER(), 0);
}
...
/* In 2.0, we have to init masq here. */
#ifdef CONFIG_IP_MASQUERADE
        /*
